...
this is the first loop. Observe that in fact, \textsc{ShockMinimization} does
not compute pseudo-closed sets but quasi-closed sets. All pseudo-closed sets are included in the resulting $\I$ after this first step. Then, we use
\belemp{lectic ordering} to have a $\subseteq$-compatible way to process 
implications before building $\I_c$.
...
The advantages of lectic ordering is to allow for easier checking of pseudo-closeness recursive property: if we test sets in lectic order,
...
get $\I$ swapped as follows: $ d \imp db, \ c \imp ca, \ ab \imp abcde, \ abcde \imp abcdef$.
...
\in \I$ (distinct) are such that $C \equiv A$ and $A \ddv C$, then we can remove $A \imp B$ from $\I$ and replace $C \imp D$ by $C \imp D \cup B$ without 
altering $\Sg^{\I}$.
\end{proposition}
...
\paragraph{Example} Let us use the example we presented when studying the first algorithm, but this time, applying formally Maier's algorithm on it. Hence,
...
\subsection{Representing implications as Horn clauses}
...
\noindent In practice, this procedure is likely to be somehow random because
of the oracle. To get rid of non-determinism, one can derive from \textsc{AngluinAlgorithm} the algorithm \textsc{AFPMinimization}.
...
\begin{itemize}
	...
	\item[-] \textsc{AFPMinimization}, with an unknown time complexity.
\end{itemize}
...
relies on quite different structure. The most interesting difference with the two previous algorithms (or in fact all other algorithms but \textsc{AusielloMinimization} being based on \textsc{MaierMinimization}) is that the resulting implication base may not be the Duquenne-Guigues basis. Still, it uses redundancy elimination even though an implication being redundant in \textsc{MinCover} may not be considered as redundant in Maier's terms (because of right-closure in the latter one). Nevertheless, we still
have a common point within those three first procedures: we are minimizing 
a given basis. Quite the opposite, \textsc{BercziMinimization} and \textsc{AFPMinimization} are building a canonical representation of an input
basis. Speaking of theoretical complexity, it seems like those approaches are not the most efficient. Still, we may keep in mind that we based our study of complexity on \textsc{LinClosure} which may not be the fastest closure algorithm in practice (see \cite{bazhanov_optimizations_2014}). 
...
heavily using quasi-closure of FCA community (\textsc{MinCover}, \textsc{DuquenneMinimization}), moving to Maier's database approach and Ausiello graphical representation. Eventually we studied algorithms from boolean logic and query learning with \textsc{BercziMinimization} and \textsc{AFPMinimization}. After having studied their operations we will in
the next chapter implement and compare them under practical computations.
