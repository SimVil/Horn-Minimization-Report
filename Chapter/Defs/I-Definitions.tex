...
\subseteq)$ described previously in figure .
...
\noindent Defining equivalence classes around closed sets, pseudo-closed sets
are then minimal quasi-closed sets among their equivalence class. The interest
of quasi-closure is that it can be seen out of the cube we are working on,
as we shall see in next example.
...
consider. In fact it represents what we call in lattice theory the 
\belemp{ideal} or \belemp{down-set} generated by $c$. It appears that the only
distinct subset of $c$ is $\emptyset$, which is closed. $c$ itself is also
not closed. Hence $c$ is indeed quasi-closed. 

\begin{figure}[ht]
	\input{Pictures/I-def-QCS.tex}
\end{figure}
