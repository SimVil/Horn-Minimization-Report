...
find less cases where a measure is lower than its ancestors. We should keep
this explanation in mind for all upcoming results, for all algorithms.
...
\begin{figure}[H]
	\input{Pictures/III/Duquenne/Operators_BFixed.tex}
\end{figure}
...
\begin{figure}[H]
	\input{Pictures/III/Duquenne/Operators_SFixed.tex}
\end{figure}
...
\begin{figure}[H]
	\input{Pictures/III/Duquenne/RealData.tex}
\end{figure}

\vspace{1.2em}

We represented in those bar plots the time required by both studied versions of \textsc{DuquenneMinimization} to minimize the basis we derive from our bag or real datasets. We did not represent the SPECT dataset because most of the values
...
Those tests allowed to emphasize on a practical desirable property of \textsc{MaierMinimization}. Actually we first tested the two versions
for $|\Sg| = 100$ and growing $|\B|$ and it turned out that both versions
had similar execution time (up to 10\% of difference from one to another,
for results not exceeding 6 seconds of executions) even though \textsc{LinClosure} may be the slowest in general. This supposes that the first step always done by \textsc{Closure} may reduce bases so that subsequent steps
are sped up. The difference however increased when we set $|\Sg|$ to 1000
as table ... shows.



\noindent When $|\B|$is fixed the difference is much more visible as one can observe in figure \ref{fig:Maier-BFixed}.
...
\begin{figure}[H]
	\input{Pictures/III/Maier/RealData.tex}
\end{figure}
...
\begin{figure}[H]
	\input{Pictures/III/AFP/ClosureOperators.tex}
\end{figure}
